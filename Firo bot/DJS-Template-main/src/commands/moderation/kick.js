...
module.exports.run = async (client, message, args) => {
    ...
    if (member.roles.highest.position >= message.member.roles.highest.position && message.guild.owner.id !== message.author.id) return message.channel.send('You can\'t kick someone that has a higher role or the same position than you.');
    if (member.roles.highest.position >= message.guild.me.roles.highest.position && message.guild.owner.id !== client.user.id) return message.channel.send('I can\'t kick someone that has a higher role or the same position than me.');
    ...
    member.send(`You got kicked from ${message.guild.name} for ${reason}`).catch(err => {});
